...
func initDatabase(cfg *config.Config) (*gorm.DB, error) {
	...
	// Manually ensure embeddings table exists (since migration 000002 was skipped)
	if dbDriver == "postgres" {
		logger.Infof(context.Background(), "Ensuring embeddings table exists...")
		
		// Create extensions
		_, err := db.Exec("CREATE EXTENSION IF NOT EXISTS vector")
		if err != nil {
			logger.Warnf(context.Background(), "Failed to create vector extension: %v", err)
		}
		_, err = db.Exec("CREATE EXTENSION IF NOT EXISTS pg_trgm")
		if err != nil {
			logger.Warnf(context.Background(), "Failed to create pg_trgm extension: %v", err)
		}
		...
		`)
		if err != nil {
			logger.Warnf(context.Background(), "Failed to create embeddings table: %v", err)
		} else {
			// Create indexes
			_, err = db.Exec("CREATE UNIQUE INDEX IF NOT EXISTS embeddings_unique_source ON embeddings(source_id, source_type)")
			if err != nil {
				logger.Warnf(context.Background(), "Failed to create embeddings_unique_source index: %v", err)
			}
			
			_, err = db.Exec("CREATE INDEX IF NOT EXISTS idx_embeddings_is_enabled ON embeddings(is_enabled)")
			if err != nil {
				logger.Warnf(context.Background(), "Failed to create idx_embeddings_is_enabled index: %v", err)
			}
			
			_, err = db.Exec("CREATE INDEX IF NOT EXISTS idx_embeddings_knowledge_base_id ON embeddings(knowledge_base_id)")
			if err != nil {
				logger.Warnf(context.Background(), "Failed to create idx_embeddings_knowledge_base_id index: %v", err)
			}
			
			// Try to create HNSW indexes (may fail if pgvector is not available)
			_, err = db.Exec(`
				CREATE INDEX IF NOT EXISTS embeddings_embedding_idx_3584 ON embeddings 
				USING hnsw ((embedding::halfvec(3584)) halfvec_cosine_ops) 
				WITH (m = 16, ef_construction = 64) 
				WHERE (dimension = 3584)
			`)
			if err != nil {
				logger.Warnf(context.Background(), "Failed to create HNSW index for 3584: %v", err)
			}
			
			_, err = db.Exec(`
				CREATE INDEX IF NOT EXISTS embeddings_embedding_idx_798 ON embeddings 
				USING hnsw ((embedding::halfvec(798)) halfvec_cosine_ops) 
				WITH (m = 16, ef_construction = 64) 
				WHERE (dimension = 798)
			`)
			if err != nil {
				logger.Warnf(context.Background(), "Failed to create HNSW index for 798: %v", err)
			}
			
			logger.Infof(context.Background(), "Embeddings table setup completed!")
		}
	}
